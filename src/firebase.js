...
async function listCandidateAssessments(uid, email = "") {
  requireFirebase();
...
async function getCandidateAssessment(assessmentId, uid, email = "") {
  requireFirebase();
...
async function startCandidateAssessment(assessmentId, uid) {
  ...
  await setDoc(doc(db, collections.assessments, assessmentId), {
    ...
    currentStage: 1,
    technicalStartedAt: serverTimestamp(),
...
async function submitCandidateAssessment(assessmentId, answers, meta = {}) {
  ...
  await setDoc(doc(db, collections.assessments, assessmentId), {
    ...
    disc: discScore ? `${discScore}%` : "Submitted",
