...
export async function sendCandidateAccountCreatedEmail(candidate = {}) {
  ...
  const response = await fetch(emailApiUrl, {
    ...
    body: JSON.stringify({
      ...
      data: {
        name: displayName || firstName,
        firstName,
        roleTitle: job.title || job.role || job.openingTitle || "this role",
        openingCode: job.code || job.id || "",
        actionUrl: "https://talent.nearwork.co"
      }
    })
  });
...
function toAtsCandidate(uid, data) {
  ...
  return {
    ...
    expectedSalaryUSD: Number(data.expectedSalaryUSD || 0) || null,
    expectedSalaryCOP: Number(data.expectedSalaryCOP || 0) || null,
...
async function applyToJob(uid, job) {
  ...
  const application = {
    ...
    candidateCode,
    candidateEmail: candidate?.email || job.candidateEmail || "",
    candidateName: candidate?.name || job.candidateName || "",
    openingCode: code,
    jobId: code,
    openingTitle: job.title || job.role || "Untitled role",
  ...
  sendCandidateApplicationSubmittedEmail(candidate, job).catch(() => null);
}
