...
const colombiaLocations = {
  ...
  "Cundinamarca": ["Chía", "Soacha", "Zipaquirá", "Cajicá", "Facatativá"],
...
function skillOptionsMarkup(selectedSkills) {
  return Object.entries(skillGroups).map(([group, skills]) => `
    <optgroup label="${escapeAttr(group)}">
      ${skills.map((skill) => `<option value="${escapeAttr(skill)}" ${selectedSkills.includes(skill) ? "selected" : ""}>${skill}</option>`).join("")}
    </optgroup>
  `).join("");
}
...
async function loadDashboard(user) {
  ...
  try {
    ...
    setState({
      ...
      loading: false,
      view: "dashboard",
      activePage: isNewAccount && candidate?.onboarded !== true ? "onboarding" : pageFromPath()
    });
  } catch (error) {
    console.warn(error);
    setState({
...
function renderOverview() {
  return `
    ${state.candidate?.onboarded === true ? "" : `
      <section class="completion-banner">
        <div><strong>Finish your candidate profile</strong><p>Add role, city, salary, and skills so Nearwork can match you to the right openings.</p></div>
        <button class="primary-action fit" data-page="onboarding">${icon("arrow-right")} Finish setup</button>
      </section>
    `}
...
function renderMatches() {
  ...
  const visibleJobs = state.matchesFiltered ? filteredJobs : state.jobs.map(normalizeRole);
...
function renderRecruiter() {
  ...
  const bookingUrl = recruiter.bookingUrl || state.candidate?.recruiterBookingUrl || "mailto:talent@nearwork.co?subject=Schedule%20a%20Nearwork%20recruiter%20call";
  return `
    <section class="content-grid">
  ...
  return `
    <section class="section-block">
      ...
      <form id="profileForm" class="profile-form">
        ...
        <label>Role applying for<select name="targetRole"><option value="">Select a role</option>${roleOptions()}</select></label>
        <label>Department
        ...
        <label>Target monthly salary (USD)<input id="salaryInput" name="salary" value="${escapeAttr(normalizedSalary.salary || "")}" placeholder="1000, USD1000, or 1000.00" /></label>
        <label>LinkedIn <span class="optional-label">optional</span><input name="linkedin" value="${escapeAttr(state.candidate?.linkedin || "")}" placeholder="https://linkedin.com/in/..." /></label>
        <label class="wide">Skills
          <select name="skills" id="skillsSelect" multiple size="9">
            ${skillOptionsMarkup(skills)}
          </select>
          <small class="field-hint">Hold Command or Ctrl to select multiple skills. Skills are grouped by function.</small>
        </label>
...
function profileCompletion() {
  const fields = ["name", "targetRole", "department", "city", "english", "salary", "summary"];
...
function bindDashboardEvents() {
  ...
  document.querySelector("#filterMatches")?.addEventListener("click", () => {
    setState({ matchesFiltered: !state.matchesFiltered, message: "" });
  });
  ...
  document.querySelector("#profileForm")?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (form.get("mode") === "onboarding") {
        window.history.pushState({ page: "overview" }, "", "/");
        setState({ candidate: { ...state.candidate, ...data }, activePage: "overview", message: "Profile complete. Welcome to Talent." });
      } else {
