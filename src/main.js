    ...
    try {
      if (currentIndex + 1 >= questions.length) {
        ...
        const completedAssessments = (state.assessments || []).map((item) => item.id === assessmentId
          ? { ...item, answers, status: "completed", score: Math.round((scores.technicalScore * 0.75) + (scores.discScore * 0.25)), technical: scores.technicalScore, disc: `${scores.discScore}%`, progress: `${questions.length}/${questions.length}` }
          : item);
