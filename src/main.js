  ...
  return `
    ${complete ? "" : `
      <section class="completion-banner">
        ...
        <button class="primary-action fit" data-page="profile">${icon("arrow-right")} Finish setup</button>
      </section>
    `}
    <section class="hero-card">
      <div><p class="eyebrow">Nearwork talent</p><h2>${firstName()}, your profile is ${profileCompletion()}% ready.</h2><p>${complete ? "Browse current openings and keep your CV ready for applications." : "Complete your profile, keep a tailored CV ready, and follow your process from application to decision."}</p></div>
      <button class="primary-action fit" data-page="${complete ? "matches" : "profile"}">${icon("arrow-right")} ${complete ? "View matches" : "Complete profile"}</button>
    </section>
