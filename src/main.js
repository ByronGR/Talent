...
function skillCardsMarkup(selectedSkills) {
  ...
  `).join("") + `
    <fieldset class="skill-group">
      <legend>Other</legend>
      <label>Other skills
        <input name="otherSkills" value="${escapeAttr(otherSkillsText())}" placeholder="Type extra skills, separated by commas" />
      </label>
    </fieldset>
  `;
}
...
async function loadDashboard(user) {
  ...
  try {
    const [candidateResult, applicationsResult, jobsResult, assessmentsResult] = await Promise.allSettled([
      getCandidateForAuthUser(user),
      listCandidateApplications(user.uid),
      listOpenJobs(),
      listCandidateAssessments(user.uid, user.email)
    ]);
    ...
    let assessments = assessmentsResult.status === "fulfilled" ? assessmentsResult.value : [];
    ...
    if (directAssessmentId && !assessments.some((assessment) => assessment.id === directAssessmentId)) {
      const directAssessment = await getCandidateAssessment(directAssessmentId, user.uid, user.email).catch(() => null);
...
function renderAssessment() {
  ...
  const activeAssessment = directId
    ...
    : assessments.find((assessment) => ["sent", "started"].includes(String(assessment.status || "").toLowerCase())) || assessments[0];
  ...
  if (activeAssessment) {
    ...
    const currentIndex = Math.min(Number(activeAssessment.currentQuestionIndex || 0), Math.max(questions.length - 1, 0));
    ...
    return `
      <section class="assessment-hero">
        <div>
          ...
          <p>${completed ? "This assessment has been submitted." : expired ? "This assessment link has expired." : "This assessment has 2 stages. Stage 1 is technical, Stage 2 is DISC. You must be logged in to complete it."}</p>
        </div>
        <button class="primary-action fit" id="startAssessment" type="button" ${completed || expired ? "disabled" : ""}>${icon(started ? "play" : "clipboard-check")} ${started ? "Continue assessment" : "Start assessment"}</button>
      </section>
      <section class="info-grid">
        ${infoCard("Stage 1", "50 technical multiple-choice questions. 60 minutes.")}
        ${infoCard("Stage 2", "20 DISC multiple-choice questions. 30 minutes.")}
      ...
      <section class="section-block page-gap" id="assessmentWorkspace">
        <div class="section-heading"><div><p class="eyebrow">${completed ? "Results" : `Stage ${currentStage} of 2`}</p><h2>${completed ? "Assessment result" : `${currentIndex + 1} of ${questions.length || 70}`}</h2></div></div>
        ${completed ? renderAssessmentResult(activeAssessment) : expired ? emptyState("Assessment expired", "This unique assessment link is no longer available. Contact Nearwork if you need help.") : renderAssessmentQuestions(activeAssessment, started)}
      </section>
    `;
  }
...
function renderAssessmentQuestions(assessment, started) {
  if (!started) {
    return `
  ...
  const currentIndex = Math.min(Number(assessment.currentQuestionIndex || 0), Math.max(questions.length - 1, 0));
  ...
  return `
    <form id="assessmentQuestionForm" class="stacked-form">
      <div class="match-note"><strong>${escapeAttr(question.part || question.type)}</strong> · ${escapeAttr(question.bank || "")} · ${question.stage === 1 ? "60 minutes" : "30 minutes"}</div>
      <label>
        ${escapeAttr(question.q || "")}
        <div class="skill-picker">
      ...
      <p class="field-hint">${finishesStage ? "After this answer, you finished Stage 1 and will enter Stage 2." : "Your progress saves after every question. If you refresh, you will return here."}</p>
      <div class="job-footer">
        ...
        <button class="primary-action fit" type="submit">${currentIndex + 1 >= questions.length ? "Submit assessment" : "Save and continue"}</button>
      </div>
    </form>
  `;
}
...
function renderAssessmentResult(assessment) {
  return `
    <div class="summary-grid">
      ${metricCard("Overall", `${assessment.score || 0}%`, "sparkles")}
      ${metricCard("Technical", `${assessment.technical || 0}%`, "clipboard-check")}
      ${metricCard("DISC", String(assessment.disc || "Submitted"), "users")}
      ${metricCard("Progress", assessment.progress || "70/70", "timer")}
    </div>
    ${emptyState("Assessment submitted", "Nearwork is reviewing your answers. Your results are saved to your profile.")}
  `;
}
...
function scoreAssessment(assessment, answers) {
  ...
  return {
    ...
    discScore: disc.length ? Math.round((disc.filter((q) => answers[q.id]).length / disc.length) * 100) : 0
  };
}
...
function bindDashboardEvents() {
  ...
  document.querySelector("#prevAssessmentQuestion")?.addEventListener("click", async () => {
    ...
    const prevIndex = Math.max(0, Number(assessment?.currentQuestionIndex || 0) - 1);
  ...
  document.querySelector("#assessmentQuestionForm")?.addEventListener("submit", async (event) => {
    ...
    const assessment = (state.assessments || []).find((item) => item.id === assessmentId);
    const questions = assessment?.questions || [];
    const currentIndex = Number(assessment?.currentQuestionIndex || 0);
    const question = questions[currentIndex];
    const answer = new FormData(event.currentTarget).get("answer");
    if (!question || answer === null) {
      setState({ message: "Select an answer before continuing." });
      return;
    }
    const answers = { ...(assessment.answers || {}), [question.id]: { value: Number(answer), answeredAt: new Date().toISOString() } };
    try {
  ...
  document.querySelector("#profileForm")?.addEventListener("submit", async (event) => {
    ...
    const data = {
      ...
      skills: [...new Set([...form.getAll("skills"), ...parseOtherSkills(form.get("otherSkills"))])],
      otherSkills: parseOtherSkills(form.get("otherSkills")),
