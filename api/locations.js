export default async function handler(req, res) {
  ...
  const table = process.env.AIRTABLE_LOCATION_TABLE || process.env.AIRTABLE_COLOMBIA_TABLE || 'Table 1';
  ...
  for (const record of records) {
    ...
    const department = String(
      ...
      || fields.Departamento
      || fields.Department
      || fields.AIRTABLE_DEPARTMENT_FIELD
      || ''
    ).trim();
    const municipality = String(
      ...
      || fields.Municipio
      || fields.Municipality
      || fields.AIRTABLE_MUNICIPALITY_FIELD
      || ''
    ).trim();
